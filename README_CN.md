...
<p align="center">
  <a href="#快速开始">快速开始</a> |
  <a href="#安装">安装</a> |
  <a href="#为什么选择-mchact">为什么选择 mchact</a> |
  <a href="#工作原理">架构</a> |
  <a href="#文档">文档</a>
</p>
...
mchact 是一个面向聊天渠道的智能体运行时。它提供统一的渠道无关 agent loop、provider 无关的 LLM 抽象层，以及可以跨 Telegram、Discord、Slack、飞书/Lark、IRC、Web 等渠道复用的持久化运行时。
...
- `ghcr.io/mchact/mchact:<版本号>`
- `docker.io/mchact/mchact:latest`，前提是仓库已配置 Docker Hub 发布凭据
...
- `mchact.config.yaml`：把配置保留在容器外，方便修改
- `data/`：持久化数据库、会话、记忆、技能和运行时数据
- `tmp/`：给容器内临时文件一个明确的可写目录

镜像入口是 `mchact`，因此可以直接覆盖子命令：
...
<p align="center">
  <img src="docs/assets/readme/mchact-architecture.svg" alt="mchact 架构总览" width="96%" />
</p>

## 博客文章

关于项目架构与设计取舍的介绍文章：**[Building mchact: An Agentic AI Assistant in Rust That Lives in Your Chats](https://mchact.ai/blog/building-mchact)**
...
<p align="center">
  <img src="docs/assets/readme/memory-architecture.svg" alt="mchact 记忆架构图" width="92%" />
</p>

mchact 通过 `AGENTS.md` 文件维护持久化记忆：
...
另外，mchact 也会把结构化记忆写入 SQLite（`memories` 表）：
...
<p align="center">
  <img src="docs/assets/readme/skills-lifecycle.svg" alt="mchact 技能生命周期图" width="92%" />
</p>

mchact 支持 [Anthropic Agent Skills](https://github.com/anthropics/skills) 标准。技能是为特定任务提供专业能力的模块化包。
...
这样可以在不改 mchact 核心 agent loop 的情况下，通过外部桥接服务扩展远程终端能力。
...
[Peekaboo](https://github.com/steipete/Peekaboo) 是一个 macOS 桌面自动化 MCP server。mchact 可通过 `stdio` 直接接入（无需修改运行时代码）。
...
mchact 也可以通过 `stdio` 接入 Windows 桌面自动化 MCP server：
...
注意：部分 Windows MCP 项目只提供 `sse` transport。mchact 当前仅直接支持 `stdio` 与 `streamable_http`，SSE-only 服务需先通过协议桥接后再接入。

## 计划与执行

<p align="center">
  <img src="docs/assets/readme/plan-execute.svg" alt="mchact 计划执行流程图" width="92%" />
</p>
...
<p align="center">
  <img src="docs/assets/readme/task-scheduler.svg" alt="mchact 定时任务流程图" width="92%" />
</p>
...
当 `web_enabled: true` 时，mchact 会启动本地 Web UI（默认 `http://127.0.0.1:10961`）。
...
`mchact gateway call` 会按以下优先级读取连接配置：
...
> **新功能：** 现在支持交互式问答配置（`mchact config`），并且在 `start` 时若缺少必需配置会自动进入配置流程。
...
2. 发送 `/newbot`
3. 输入机器人的显示名称（例如 `My mchact`）
4. 输入用户名（必须以 `bot` 结尾，例如 `my_mchact_bot`）
...
对于 `openai-codex`：你可以先运行 `codex login`，mchact 会读取 `~/.codex/auth.json`（或 `$CODEX_HOME/auth.json`）里的 OAuth 凭据。也可以在使用 OpenAI 兼容中转端点时配置 `api_key`。默认 base URL 是 `https://chatgpt.com/backend-api`。

如果你更喜欢手工配置，也可以直接写 `mchact.config.yaml`：
...
- Windows 使用由 `mchact.exe` 直接托管的原生 Windows Service。运行 `mchact gateway install` 前，请先准备好 `mchact.config.yaml`，并在管理员终端中执行 gateway 服务命令
- 运行日志写入 `<data_dir>/runtime/logs/`
- 日志按小时分片：`mchact-YYYY-MM-DD-HH.log`
- 超过 30 天的日志会自动删除

## 配置项

所有配置都在 `mchact.config.yaml` 中。
...
| `data_dir`                                     | 否   | `~/.mchact`             | 数据根目录（运行时数据在 `data_dir/runtime`，技能在 `data_dir/skills`）                                      |
| `working_dir`                                  | 否   | `~/.mchact/working_dir` | 工具默认工作目录；`bash/read_file/write_file/edit_file/glob/grep` 的相对路径都以此为基准                     |
...
| `channels.irc.real_name`                       | 否   | `"mchact"`              | IRC real name（USER 命令字段）                                                                               |
...
- 如果未配置，则默认使用 `data_dir=~/.mchact`、`skills_dir=<data_dir>/skills`、`working_dir=~/.mchact/working_dir`。
...
- 可选加固：
  - `~/.mchact/sandbox-mount-allowlist.txt`：沙箱挂载路径白名单。
  - `~/.mchact/sandbox-path-allowlist.txt`：文件工具路径白名单。
...
mchact 的核心智能体循环是渠道无关的。新增平台时，重点是实现适配器层：
...
mchact 支持基于 OpenTelemetry (OTLP) 的可观测能力，并与 [Langfuse](https://langfuse.com/) 深度集成。你可以追踪完整的 `agent_run`，查看 `llm_generation` / `tool_execution` 子链路，并监控 Token 消耗。
...
3. **编辑 mchact 配置** `mchact.config.yaml`
4. **重启 mchact**
...
- **Docker 网络误区**
  ...
  - mchact 在容器里运行时，请改用容器可达域名（如 `http://langfuse-web:3000`）
- **改完配置后看不到数据**
  - 修改配置后必须重启 mchact
  - 发送新请求再到 Langfuse 查看
